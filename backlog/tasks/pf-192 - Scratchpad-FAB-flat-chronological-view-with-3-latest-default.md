...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 All entries (general + step-specific) rendered in a single flat list sorted by timestamp descending (newest first)
- [ ] #2 Only the 3 most recent entries visible by default
- [ ] #3 A 'Show N more' toggle expands the remaining entries below the initial 3
- [ ] #4 Each entry displays its origin: step name (from stepNames prop) for step entries, 'general' for general notes
- [ ] #5 Entry type badge (note/rating/reminder) and YYYY-MM-DD HH:MM timestamp still shown per entry
- [ ] #6 Step-grouped collapsible section removed entirely
- [ ] #7 No changes to scratchpad data storage format — display-only refactor
