...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Description preserves full architecture map: ProductionPlan + 7 lenses (Plan/Price/Labels/Schedule/Sell/Financials/DataHealth) + cost factor stack + hard requirements (local-only, agent-in-loop, provenance, confidence).
- [ ] #2 9 candidate slices (S0-S8) identified in description.
- [ ] #3 S0 (Plan skeleton) groomed as first subtask with shippable ACs.
- [ ] #4 Other slices remain as Draft tasks (DRAFTs 80-94) until each individually groomed.
- [ ] #5 PF-255 (closed) referenced as foundational research, not invalidated.
- [ ] #6 Labels applied: epic, bakery-ops, umbrella.
<!-- AC:END -->
