...
| F43 | Help tooltips on interactive elements | Every interactive or jargon-bearing UI element (icons, badges, sliders, computed values, tag chips, legend swatches, phase blocks) must surface a help tooltip on hover/focus explaining what it is or does. Native `title=` acceptable as a fallback; use the project's `<HelpTooltip>` component (DRAFT-89) once shipped. Verified by manual review during HITL gates for any styling/UI task. | 2026-05-12 |
